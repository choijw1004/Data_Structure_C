...
void sequential_print(type_ptr_datanode HEAD) {
	type_ptr_datanode dataPtr = HEAD;

	FILE* fp;

	fp = fopen("sequential_print.txt", "w");

	while (dataPtr->link != NULL) {
		for (int i = 0; i < dataPtr->fill_cnt; i++) {
			fprintf(fp, "%s %d\n", dataPtr->rec[i].name, dataPtr->rec[i].leng);
		}
		dataPtr = dataPtr->link;
	}
	fclose(fp);
}
...
type_ptr_datanode get_datanode(char* name)
{
	...
	if (ROOT->ptri[0] != NULL) {	// ROOT 가 리프노드가 아님. 
		...
		do {
			...
			curr = curr->/* Fill your code */ptri[i];	// 자식으로 내려 감.
			if (curr->ptri[0] == NULL)
	...
	curr_d = curr->/* Fill your code */ptrd[i]; // 데이타노드로 내려 감. curr_d 은 데이타노들 가리킴.
...
int range_search(char* key1, char* key2)
{
	...
	if (ptr1 == ptr2) { // start key 와 end key 가 동일 데이타노드에 속한다.
		...
		for (j = i; j < ptr1->fill_cnt && strcmp(ptr1->/* Fill your code */rec[j].name, key2) <= 0; j++) {
	...
	for (j = i; j < ptr1->fill_cnt; j++) { // i 이후의 모든 레코드들을 출력한다.
		fprintf(fp, "%s %d\n", /* Fill your code */ptr1->rec[j].name, /* Fill your code */ptr1->rec[j].leng);
		fflush(fp);
		count++;
	}

	// ptr2 보다 앞의 노드들은  노드내의 모든 레코드들을 모두 출력한다.
	tptr = ptr1->/* Fill your code */link; // 다음 노드로 간다.
	while (tptr != /* Fill your code */ptr2) {
		// tptr 노드 내의 모든 레코드들을 출력한다.
		for (i = 0; i < tptr->fill_cnt; i++) {
			fprintf(fp, "%s %d\n", /* Fill your code */tptr->rec[i].name, tptr->rec[i].leng);
			fflush(fp);
			count++;
		}
		// tptr 을 다음 노드를 가리키게 한다.
		tptr = tptr->/* Fill your code */link;
	}

	// ptr2 노드의 내용을 처리할 차례이다. key2 이전(key2 포함)의 레코드들을 출력한다.
	for (i = 0; i < /* Fill your code */ptr2->fill_cnt && strcmp(ptr2->rec[i].name, key2/* Fill your code */) <= 0; i++) {
...
int insert_arec_b_plus_tree(type_rec in_rec) {	//하나의 레코드를 삽입.
	...
	{   // 초기상황임.
		...
		{   // 첫 데이타노드가 full 임. split 가 필요함.
			...
			for (j = 0; j < i; j++) // i 이전 부븐을 빅노드로 가져옴.
				bnode_data.rec[j] = HEAD->/* Fill your code */rec[j];
			...
			HEAD->link = /* Fill your code */new_ptrd;	// 연결리스트에 새 노드를 연결함.
	...
	{
		for (i = 0; i < fc; i++) {
			if (strcmp(in_key, curr_d->rec[i].name)/* Fill your code */ < 0)
				break;  // i 가 결정됨.
			else if (strcmp(in_key, curr_d->rec[i].name) == 0)
			{
				printf("동일키 이미 존재로 삽입 실패!\n");
		...
		// i 부터 우측을 한칸씩 우측으로 시프트함.
		for (j = fc; j > i; j--)
			curr_d->rec[j] = curr_d->/* Fill your code */rec[j - 1];
	...
	{ 	// 첫 데이타노드가 full 임. split 가 필요함. big data node 를 준비함
		...
		for (j = 0; j < i; j++) // i 이전 부분을 빅노드로 가져옴.
			bnode_data.rec[j] = curr_d->/* Fill your code */rec[j];
		bnode_data.rec[j] = in_rec;
		j++;
		while (i < MAXD_data) {
		...
		new_ptrd = (type_ptr_datanode)malloc(sizeof(type_data_node));
		for (i = 0; i < D_data; i++)	// 나머지 뒤는 새로운 노드에 넣음.
			new_ptrd->rec[i] = bnode_data.rec[/* Fill your code */i + 1 + D_data];//here
		new_ptrd->fill_cnt = D_data;
		new_ptrd->link = NULL;

		// 시퀀스셋 연결리스트에 이 새로운 노드를 curr 다음 노드가 되게 끼워 넣어야 한다.
		new_ptrd->link = /* Fill your code */curr_d->link;
	...
	do {
		...
		else 	//   curr 에 빈자리가 없다. 인덱스노드의 빅노드 이용하여 split & elevation 을 수행한다.
		{
			...
			// 쌍의 원소2 인  포인터를넣어 준다.
			if (curr->ptri[0] == NULL) { // curr 는 리프노드이므로 type-1 삽입이다.
				bnode_index.ptrd[j + 1] = /* Fill your code */child_d;   // 자식 포인터를 ptrd 에 삽입 
				bnode_index.ptri[j + 1] = NULL;  // 이것은 넣지 않아도 문제는 없다.
			}
			else { // curr 는 리프노드가 아니다. 고로 type-2 삽입이다. 자식 포인터를 ptri 에 넣는다.
				bnode_index.ptri[j + 1] = /* Fill your code */child;
			...
			for (i = 0; i < /* Fill your code */D_idx; i++) {
			...
			curr->ptrd[i] = /* Fill your code */bnode_index.ptrd[i];
			curr->fill_cnt = D_idx;

			new_ptri = (type_ptr_idxnode)malloc(sizeof(type_idx_node)); // 빅노드의 후반부를 새로운 노드에 넣는다. 

			for (i = 0; i </* Fill your code */D_idx + 1; i++) {	//뒤의 반을 새노드에 저장 
			...
			new_ptri->ptri[i] = bnode_index.ptri[/* Fill your code */i + 1 + D_idx];	// 맨 마지막 포인터를 옮긴다.
			new_ptri->ptrd[i] = bnode_index.ptrd[/* Fill your code */i + 1 + D_idx];
