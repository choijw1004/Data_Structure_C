	...
	if (Hashtable[HA].name[0] == '\0') { // 홈주소가 비어 있음. 여기에 넣고 종료.
		...
		do {
			...
			if (Hashtable[curr].link == -1)
				...
				curr = /* Fill your code */Hashtable[curr].link;;
		} while (1);
		...
		// 새로 넣은 위치를 chain 에 연결한다.
		Hashtable[curr].link = /* Fill your code */empty_loc;
		return nprove;
	} //else.
} // end of insert_rec
...
float compute_average_number_of_probes_per_search() {
	...
	while (1)
	{
		...
		if (loc != -1) {
			n_search_done += 1;
			total_probes += /* Fill your code */nprobe;
		}
	}
	average_probe = (float)total_probes / /* Fill your code */n_search_done;
	fclose(fp);
...
int del_start(int s, int* chain_split)
{
	...
	while (curr != -1)
	{
		if (hash(Hashtable[curr].name) == s) {
			prev_found = /* Fill your code */prev; // 찾은 곳의 선행 주소도 저장.
			found = curr; // 찾은 장소 저장.
		}
		// 다음 주소로 전진한다.
		prev = curr;
	...
	if (found == -1) { // 홈주소가 s 인 레코드가 발견되지 않음.
		...
		nmove = 1 + del_middle(/* Fill your code */found, /* Fill your code */prev_found, chain_split);
		return nmove;
	}
} // end of del_starat.
...
int del_middle(int s, int p, int* chain_split)
{
	...
	if (found == -1) { // 찾기 실패. 위치 s 를 체인에서 skip 하고, s 의 레코드를 제거한다.
		Hashtable[p].link = /* Fill your code */Hashtable[s].link;  // skip s.
		...
		while (curr != -1) {
			...
			if (HA_curr_belongs_to_D_s == 0) {  // D(s) 에 속하지 않는 HA 를 가진 원소 발견!
				found = curr;
				prev_found = /* Fill your code */prev;
			}
		...
		if (found != -1) { // D(s)에 HA가 속하지 않는 원소가 존재함. 
			...
			/* Fill your code */ Hashtable[p].link = -1; // 체인 분리.
			*chain_split += 1; // 체인 분리 횟수 증가.
			nmove = del_start(/* Fill your code */s, chain_split);  // s에 있는 레코드 삭제. s 는 체인의 시작.
			return nmove;
		}
	}
} // end of del_middle.
...
int delete_rec(char* dkey, int* chain_split) {
	...
	if (strcmp(dkey, Hashtable[h].name) == 0)  // dkey is at its home address.
		nmove = /* Fill your code */del_start(h, chain_split);
	else {
		...
		if (found == -1)
		{
			...
			nmove = /* Fill your code */del_middle(curr, prev, chain_split);
	}//else.
...
void delete_multiple(int num_del_req) {
	...
	while (1)
	{
		...
		leng = strlen(line);
		line[leng - 1] = '\0';  // 마지막 newline 글자를 지운다.
		nmove = delete_rec(/* Fill your code */res, &num_split);
		if (nmove >= 0)
