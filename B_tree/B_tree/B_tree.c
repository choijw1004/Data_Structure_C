...
int insert_arec(ele in_rec) {   //하나의 레코드를 삽입  key = 회사명  
    ...
    if (!root) {   // root가 NULL이면 btree가 비어있음. 맨 첫 노드를 만들어 여기에 넣는다.
        root = (nodeptr)malloc(sizeof(nodeptr));         // nodeptr형태로 node크기만큼 할당받아 시작주소는 root가 가짐   
    ...
    int k;
    do {
        for (k = 0; k < curr->fill_cnt - 1; k++) {
            if (strcmp(in_rec.name, curr->rec[k + 1].name) < 0)
                break;
            else
                ;
        }
    ...
    do {
        ...
        if (curr->fill_cnt < MAXK) {
            for (i = 0; i < curr->fill_cnt; i++)
                if (strcmp(in_rec.name, curr->rec->name) < 0)
                    break;
            ...
            bnode.ptr[0] = NULL;
            for (j = 0; j < i; j++) {
                bnode.rec[j] = bnode.rec[j + 1];
                bnode.ptr[j + 1] = bnode.ptr[j];
            }
            ...
            if (top >= 0) {      // 스택이 emtpy 가 아닐 경우 
                ...
                tptr = (nodeptr)malloc(sizeof(nodeptr));
                tptr->rec[0] = in_rec;
                tptr->ptr[0] = NULL;
                tptr->ptr[1] = NULL;
                tptr->fill_cnt = 1;
...
void redistribution(nodeptr father, nodeptr l_sibling, nodeptr r_sibling, char wcase, int j) {
    ...
    for (k = 0; k < r_sibling->fill_cnt; k++) {
        ...
        do {
            ...
            if (r_OK || l_OK) {
             ...
             //  Let rightptr point to the right one of curr and sibling chosen to merge ;
                ...
                //   아래 빈 곳은 leftptr, rightptr 두 형제를 leftptr 형제로 합병하는 부분
                //   주의: 변수 i 가 두 형제 사이의 father 내의 중간 레코드를 가리키게 해 놓아야 함.
                //   주의: leftptr 의 fill_cnt 가 1 증가하고, rightptr 의 fill_cnt 가 1 감소함.
                leftptr->rec[leftptr->fill_cnt] = father->rec[i];
                leftptr->fill_cnt = leftptr->fill_cnt + 1;
                for (j = 0; j < rightptr->fill_cnt; j++) {
                    leftptr->rec[leftptr->fill_cnt] = rightptr->rec[j];
                    leftptr->ptr[leftptr->fill_cnt] = rightptr->ptr[j];
                    leftptr->fill_cnt = leftptr->fill_cnt + 1;
                }
                leftptr->ptr[leftptr->fill_cnt] = rightptr->ptr[j];
                free(rightptr);
